...
let trailParticles = [];
const BASE_CUBE_ASSET_BASES = ['./', '../Mods/', '../Mods Stuff/'];
const NORMAL_CUBE_MOD_ASSET_BASES = ['../Mods/Cubes/Normal Cubes/', '../Mods/Cubes/', '../Mods/', '../Mods Stuff/Cubes/Normal Cubes/', '../Mods Stuff/Cubes/', '../Mods Stuff/', './'];
const ROBTOP_CUBE_MOD_ASSET_BASES = ['../Mods/Cubes/RobTop Cubes/', '../Mods/Cubes/', '../Mods/', '../Mods Stuff/Cubes/RobTop Cubes/', '../Mods Stuff/Cubes/', '../Mods Stuff/', './'];
const GAME_CUBE_MOD_ASSET_BASES = ['../Mods/Cubes/Game Cubes/', '../Mods/Cubes/', '../Mods/', '../Mods Stuff/Cubes/Game Cubes/', '../Mods Stuff/Cubes/', '../Mods Stuff/', './'];
const NYAN_ASSET_BASES = ['../Mods/', '../Mods Stuff/', './'];
const MENU_CUBE_ICON_SOURCES = [
    '../Mods/Cubes/Normal Cubes/GD Colon Cube.png',
    '../Mods/Cubes/Normal Cubes/Vortrox Cube.png',
    '../Mods/Cubes/Normal Cubes/Nexus.png',
    '../Mods/Cubes/Normal Cubes/Tricipital.png',
    '../Mods/Cubes/RobTop Cubes/GD Icon.png',
    '../Mods/Cubes/RobTop Cubes/RobTop.png',
    '../Mods/Cubes/RobTop Cubes/RubRub.png',
    '../Mods/Cubes/Game Cubes/Avatar Aang.png',
    '../Mods/Cubes/Game Cubes/Boomlings.png',
    '../Mods/Cubes/Game Cubes/Sans.png',
    '../Mods/Cubes/Game Cubes/Batman.png',
    '../Mods/Cubes/Game Cubes/Creeper.png',
    '../Mods/Cubes/Game Cubes/Super Meat Boy.png'
];
const MENU_ICON_HITBOX = 57;
const menuCubeSpriteCache = new Map();
const BASE_CUBE_CANDIDATE_FILES = [
    'Normal Cube.png',
    'Normal Cube.PNG',
    'Normal Cube',
    'Base Cube.png',
...
const baseCubeImage = new Image();
let baseCubeImageLoaded = false;
const gdColonCubeImage = new Image();
const vortroxCubeImage = new Image();
const nexusCubeImage = new Image();
const gdIconCubeImage = new Image();
const robTopCubeImage = new Image();
const rubRubCubeImage = new Image();
const tricipitalCubeImage = new Image();
const avatarAangCubeImage = new Image();
const boomlingsCubeImage = new Image();
const sansCubeImage = new Image();
const batmanCubeImage = new Image();
const creeperCubeImage = new Image();
const superMeatBoyCubeImage = new Image();
...
const rgbMaskCtx = rgbMaskCanvas.getContext('2d', { willReadFrequently: true });
...
loadFirstAvailableAsset(baseCubeImage, getAssetCandidates(BASE_CUBE_CANDIDATE_FILES, BASE_CUBE_ASSET_BASES), () => {
    baseCubeImageLoaded = true;
});
loadFirstAvailableAsset(gdColonCubeImage, getAssetCandidates(['GD Colon Cube.png'], NORMAL_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(vortroxCubeImage, getAssetCandidates(['Vortrox Cube.png'], NORMAL_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(nexusCubeImage, getAssetCandidates(['Nexus.png'], NORMAL_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(gdIconCubeImage, getAssetCandidates(['GD Icon.png'], ROBTOP_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(robTopCubeImage, getAssetCandidates(['RobTop.png'], ROBTOP_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(rubRubCubeImage, getAssetCandidates(['RubRub.png'], ROBTOP_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(tricipitalCubeImage, getAssetCandidates(['Tricipital.png'], NORMAL_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(avatarAangCubeImage, getAssetCandidates(['Avatar Aang.png'], GAME_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(boomlingsCubeImage, getAssetCandidates(['Boomlings.png'], GAME_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(sansCubeImage, getAssetCandidates(['Sans.png'], GAME_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(batmanCubeImage, getAssetCandidates(['Batman.png'], GAME_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(creeperCubeImage, getAssetCandidates(['Creeper.png'], GAME_CUBE_MOD_ASSET_BASES));
loadFirstAvailableAsset(superMeatBoyCubeImage, getAssetCandidates(['Super Meat Boy.png'], GAME_CUBE_MOD_ASSET_BASES));
    ...
    try {
        ...
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                ...
                // Ignore faint glow alpha so bounds match the core icon footprint.
                if (a > 80) {
                    if (x < minX) minX = x;
                    if (y < minY) minY = y;
                    if (x > maxX) maxX = x;
        ...
        // Fallback for file:// security/canvas restrictions.
        return { x: 0, y: 0, w, h };
    }
}

function ensureMenuCubeSpriteEntry(src) {
    if (menuCubeSpriteCache.has(src)) return menuCubeSpriteCache.get(src);
    const entry = { img: new Image(), ready: false, bounds: null, pending: [] };
    entry.img.onload = () => {
        entry.bounds = getOpaqueBounds(entry.img);
        entry.ready = true;
        for (const canvas of entry.pending) renderMenuCubeSpriteToCanvas(src, canvas);
        entry.pending = [];
    };
    entry.img.src = src;
    menuCubeSpriteCache.set(src, entry);
    return entry;
}

function renderMenuCubeSpriteToCanvas(src, canvas) {
    const entry = ensureMenuCubeSpriteEntry(src);
    if (!entry.ready) {
        entry.pending.push(canvas);
        return;
    }
    ...
    const sw = entry.bounds.w;
    const sh = entry.bounds.h;
    const fitScale = Math.min(cw / sw, ch / sh);
    const coreW = Math.max(1, Math.round(sw * fitScale));
    const coreH = Math.max(1, Math.round(sh * fitScale));
    const side = Math.min(cw, ch, Math.max(coreW, coreH));
    const dx = Math.floor((cw - side) / 2);
    const dy = ch - side; // pin icon to bottom so all sit on the same ground line

    ctx.drawImage(entry.img, entry.bounds.x, entry.bounds.y, sw, sh, dx, dy, side, side);
}
...
function spawnMenuFakeIcon() {
    ...
    const cubeSrc = MENU_CUBE_ICON_SOURCES[Math.floor(Math.random() * MENU_CUBE_ICON_SOURCES.length)];
    renderMenuCubeSpriteToCanvas(cubeSrc, cubeCanvas);
...
function startMenuAmbient() {
    ...
    for (const src of MENU_CUBE_ICON_SOURCES) ensureMenuCubeSpriteEntry(src);
...
function drawBackground(ctx, camX) {
    ...
    for (let s of stars) {
        ...
        ctx.globalAlpha = s.alpha + Math.sin(Date.now() / 400 + s.x) * 0.25;
...
function drawGround(ctx, camX, camY = 0) {
    ...
    ctx.strokeStyle = '#6B2A9E'; ctx.lineWidth = 5;
...
function drawPlayer(ctx, camY = 0) {
    ...
    let activeCubeImage = null;
    if (gdColonCubeEnabled) activeCubeImage = gdColonCubeImage;
    else if (vortroxCubeEnabled) activeCubeImage = vortroxCubeImage;
    else if (nexusCubeEnabled) activeCubeImage = nexusCubeImage;
    else if (gdIconCubeEnabled) activeCubeImage = gdIconCubeImage;
    else if (robTopCubeEnabled) activeCubeImage = robTopCubeImage;
    else if (rubRubCubeEnabled) activeCubeImage = rubRubCubeImage;
    else if (tricipitalCubeEnabled) activeCubeImage = tricipitalCubeImage;
    else if (avatarAangCubeEnabled) activeCubeImage = avatarAangCubeImage;
    else if (boomlingsCubeEnabled) activeCubeImage = boomlingsCubeImage;
    else if (sansCubeEnabled) activeCubeImage = sansCubeImage;
    else if (batmanCubeEnabled) activeCubeImage = batmanCubeImage;
    else if (creeperCubeEnabled) activeCubeImage = creeperCubeImage;
    else if (superMeatBoyCubeEnabled) activeCubeImage = superMeatBoyCubeImage;
    else if (baseCubeImageLoaded) activeCubeImage = baseCubeImage;
    if (activeCubeImage && activeCubeImage.complete && activeCubeImage.naturalWidth > 0) {
        // Draw sprite in the exact same footprint as the normal cube.
        const drawX = -player.width / 2;
        const drawY = -player.height / 2;
        ctx.drawImage(activeCubeImage, drawX, drawY, player.width, player.height);

        // If RGB mod is enabled, tint the sprite with a cycling rainbow overlay.
        if (rgbCubeEnabled) {
            const hue = (Date.now() / 8) % 360;
            const pw = Math.max(1, Math.round(player.width));
            const ph = Math.max(1, Math.round(player.height));
            if (rgbMaskCanvas.width !== pw || rgbMaskCanvas.height !== ph) {
                rgbMaskCanvas.width = pw;
                rgbMaskCanvas.height = ph;
            }

            // Build a clean alpha mask from the icon first to prevent RGB fringe on padded/non-square icons.
            rgbMaskCtx.clearRect(0, 0, pw, ph);
            rgbMaskCtx.drawImage(activeCubeImage, 0, 0, pw, ph);
            const imgData = rgbMaskCtx.getImageData(0, 0, pw, ph);
            const data = imgData.data;
            for (let i = 0; i < data.length; i += 4) {
                if (data[i + 3] < 28) data[i + 3] = 0;
            }
            rgbMaskCtx.putImageData(imgData, 0, 0);
            rgbMaskCtx.globalCompositeOperation = 'source-atop';
            rgbMaskCtx.fillStyle = `hsla(${hue}, 95%, 55%, 0.55)`;
            rgbMaskCtx.fillRect(0, 0, pw, ph);
            rgbMaskCtx.globalCompositeOperation = 'source-over';

            ctx.drawImage(rgbMaskCanvas, drawX, drawY, player.width, player.height);
        }
    } else {
        const hue = (Date.now() / 8) % 360;
...
function drawObstacle(ctx, obs, sx, camY = 0) {
    ...
    const drawAt = (x, y) => {
        if (obs.type === 'block') {
        ...
        } else if (obs.type === 'yellowOrb' || obs.type === 'blueOrb') {
            const isBlue = obs.type === 'blueOrb';
            const glowColor = isBlue ? '#6cd6ff' : '#ffd44d';
            const fillColor = isBlue ? '#42bfff' : '#ffd84b';
            const rimColor = isBlue ? '#007bd6' : '#f5a700';
            const innerColor = isBlue ? '#b8eeff' : '#fff2a1';
            ...
            ctx.stroke();
            ctx.restore();
        } else if (obs.type === 'yellowPad' || obs.type === 'bluePad') {
            const isBlue = obs.type === 'bluePad';
            const glowColor = isBlue ? '#73ddff' : '#ffd55a';
            const bodyColor = isBlue ? '#007ecf' : '#d39100';
            const topColor = isBlue ? '#79dcff' : '#ffe27b';
            const arrowColor = isBlue ? '#d9f8ff' : '#fff4b0';
            const strokeColor = isBlue ? '#c8f2ff' : '#fff6c2';
...
function updateDeathExplosion() {
    ...
    deathExplosion.life--;
    deathExplosion.radius += 7.5;
    deathFlash = Math.max(0, deathFlash - 0.08);
...
function gameUpdate() {
    if (!gameRunning || paused || gameOver || won) {
        ...
        if (padTriggerCooldown > 0) padTriggerCooldown--;
        player.dy += gravity * gravityDirection;
        player.y += player.dy;

        player.onGround = false;
        // Only the floor is a global solid surface; upside-down cube should
        // land on block undersides, not stick to top of screen.
        ...
        for (let obs of obstacles) {
            if (obs.type !== 'yellowPad' && obs.type !== 'bluePad') continue;
            const sx = obs.worldX - cameraX;
            if (sx < -100 || sx > gameCanvas.width + 100) continue;
        ...
        if (jumpPressedThisFrame || (jumpHeld && !orbConsumedThisHold)) {
            for (let obs of obstacles) {
                if (obs.type !== 'yellowOrb' && obs.type !== 'blueOrb') continue;
        ...
        for (let obs of obstacles) {
            ...
            if (obs.type === 'spike') {
                const hitPad = getScaled(8);
                const hitTopPad = getScaled(5);
                const hitLeft = sx + hitPad;
                const hitRight = sx + obs.w - hitPad;
                const hitTop = obs.y + hitTopPad;
                const hitBottom = obs.y + obs.h - hitTopPad;
                const hitMidTop = obs.y + hitPad;
                const hitMidBottom = obs.y + obs.h - hitPad;
                const rot = normalizeRotation(obs.rotation);
                const overlapX = player.x + player.width > hitLeft && player.x < hitRight;
                const overlapY = player.y + player.height > hitMidTop && player.y < hitMidBottom;

                if (rot === 0) {
                    if (overlapX && player.y + player.height > hitTop) {
                        died = true;
                        break;
                    }
                } else if (rot === 180) {
                    if (overlapX && player.y < hitBottom) {
                        died = true;
                        break;
                    }
                } else if (rot === 90) {
                    const hitRightSide = sx + obs.w - hitTopPad;
                    if (overlapY && player.x + player.width > hitRightSide) {
                        died = true;
                        break;
                    }
                } else {
                    const hitLeftSide = sx + hitTopPad;
                    if (overlapY && player.x < hitLeftSide) {
                        died = true;
                        break;
                    }
                }
        ...
        if (!player.onGround) player.angle += 7.5;

        // GD-like vertical camera: mostly stable on ground, follows upward climbs smoothly.
        // Keep camera behavior consistent regardless of gravity direction.
        ...
        const minCameraY = -Math.round(gameCanvas.height * 0.42);
        targetGameCameraY = Math.max(minCameraY, Math.min(0, targetGameCameraY));
        gameCameraY += (targetGameCameraY - gameCameraY) * 0.12;
    ...
    if (!gameRunning || paused || gameOver || won) {
        gameCameraY += (0 - gameCameraY) * 0.14;
        updateTrail();
        if (gameOver) {
            updateParticles();
            updateDeathExplosion();
            deathRestartTimer -= 16.6667;
    ...
    // Background is drawn first so stars always stay behind ground/objects/player.
...
document.addEventListener('keydown', e => {
    ...
    if (currentScreen === 'editor' && editorRunning) {
        if (e.key === 'a' || e.key === 'ArrowLeft') editorCameraX = Math.max(0, editorCameraX - 80);
        if (e.key === 'd' || e.key === 'ArrowRight') editorCameraX += 80;
        ...
        if (e.key.toLowerCase() === 'r' && editorPanelMode === 'edit') {
...
function placeOrErase(e) {
    ...
    if (editorTool === 2) {
        ...
        if (hitIndex >= 0) {
            ...
            if (selectedObstacleIndex === hitIndex) selectedObstacleIndex = -1;
            else if (selectedObstacleIndex > hitIndex) selectedObstacleIndex--;
        }
    } else {
        // remove overlap
        obstacles = obstacles.filter(o => !(o.worldX === wx && o.y === wy));
...
function getObstacleIndexAt(worldX, worldY) {
    for (let i = obstacles.length - 1; i >= 0; i--) {
        const o = obstacles[i];
        if (worldX >= o.worldX && worldX <= o.worldX + o.w && worldY >= o.y && worldY <= o.y + o.h) return i;
    }
    return -1;
}
...
function selectObstacleAtPointer(e) {
    ...
    selectedObstacleIndex = getObstacleIndexAt(worldX, worldY);
}

window.deleteSelectedObject = function () {
    if (selectedObstacleIndex < 0 || selectedObstacleIndex >= obstacles.length) return;
...
window.setEditorPanelMode = function (mode) {
    ...
    if (editorPanelMode === 'interactable' && (editorTool < 2 || editorTool > 6)) setTool(3);
    if (editorPanelMode !== 'edit') selectedObstacleIndex = -1;
    endEditorDrag();
...
function sanitizeLevelObstacles(value) {
    ...
    const allowedTypes = new Set(['block', 'spike', 'yellowOrb', 'yellowPad', 'blueOrb', 'bluePad']);
...
function gameLoop() { gameUpdate(); if (gameRunning) requestAnimationFrame(gameLoop); }
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && (currentScreen === 'levelselect' || currentScreen === 'mods' || currentScreen === 'cheats' || currentScreen === 'cubes' || currentScreen === 'normalcubes' || currentScreen === 'robtopcubes' || currentScreen === 'gamecubes' || currentScreen === 'cubestyles' || currentScreen === 'extras')) goToMenu();
});
